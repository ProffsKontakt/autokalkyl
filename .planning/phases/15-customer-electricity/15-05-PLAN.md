...
must_haves:
  truths:
    - "Closer can select customer type (Privatperson/Foretag) from dropdown"
    - "Closer can enter kopt el with annual/monthly toggle"
    - "Closer can enter electricity price with ore/SEK unit toggle"
    - "Closer can toggle 'Har solceller?' to show/hide solar inputs"
    - "Solar section shows three-field self-consumption model when enabled"
  artifacts:
    - path: "src/components/calculations/wizard/steps/electricity-step.tsx"
      provides: "Wizard step for customer electricity inputs"
      min_lines: 200
  key_links:
    - from: "src/components/calculations/wizard/steps/electricity-step.tsx"
...
<objective>
Create the electricity input wizard step UI component.

Purpose: This is the main user-facing component for Phase 15 - where closers enter customer electricity data.
Output: Fully functional wizard step with customer type, kopt el, electricity price, and solar inputs.
</objective>
...
@.planning/phases/15-customer-electricity/15-02-SUMMARY.md
...
<task type="auto">
  <name>Task 1: Create ElectricityStep component with customer type and kopt el</name>
  <files>src/components/calculations/wizard/steps/electricity-step.tsx</files>
  <action>
Create new file `src/components/calculations/wizard/steps/electricity-step.tsx`:
...
2. File compiles without import errors
  </verify>
  <done>ElectricityStep component created with customer type dropdown, kopt el input with annual/monthly toggle, electricity price with unit toggle, and conditional solar section</done>
</task>
...
3. All store actions are called correctly
4. Solar validation shows warnings/errors appropriately
</verification>

<success_criteria>
- Customer type dropdown with Privatperson/Foretag options
- Kopt el input with annual/monthly toggle that syncs values
- Electricity price input with ore/SEK unit toggle
- "Har solceller?" toggle shows/hides solar section
- Solar section has three-field model (production, current self-consumption, projected self-consumption)
- Self-consumption input supports kWh or % mode
- Auto-suggest fills self-consumption when production is entered
- Validation warnings display for unusual values
- Summary card shows entered data
- No build errors
</success_criteria>

<output>
After completion, create `.planning/phases/15-customer-electricity/15-05-SUMMARY.md`
</output>
