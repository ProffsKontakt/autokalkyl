...
must_haves:
  truths:
    - "Salesperson can enter a custom value by clicking edit icon on any savings field"
    - "Custom values persist after page reload"
    - "Invalid input is rejected and field reverts to previous value"
    - "Reset button returns value to the calculated amount"
  artifacts:
