...
must_haves:
  truths:
    ...
    - "Prospect view displays battery price (TRANS-03)"
...
4. Verify that battery price (TRANS-03) is visible in the existing summary section (totalPriceExVat or costAfterGronTeknik). If not, add battery price display to the summary.

5. After the existing "Detailed breakdown" section (showDetailedBreakdown), add a new section for formula breakdowns:
...
Battery price is visible in the public view (TRANS-03)
pnpm tsc --noEmit passes
