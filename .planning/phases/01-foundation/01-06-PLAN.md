  ...
  <action>
    ...
    **src/actions/password-reset.ts:**
    ...
    export async function resetPassword(data: {
      ...
      if (!resetToken) {
        return { error: 'Ogiltig eller utgangen lankning' };
      }

      // Check expiration
      if (resetToken.expiresAt < new Date()) {
        await prisma.passwordResetToken.delete({ where: { id: resetToken.id } });
        return { error: 'Lankningen har gatt ut. Begr en ny.' };
      }

      // Check if already used
      if (resetToken.usedAt) {
        return { error: 'Denna lankning har redan anvants' };
      }
    ...
    export async function validateResetToken(token: string) {
      ...
      if (!resetToken) {
        return { valid: false, error: 'Ogiltig lankning' };
      }

      if (resetToken.expiresAt < new Date()) {
        return { valid: false, error: 'Lankningen har gatt ut' };
      }

      if (resetToken.usedAt) {
        return { valid: false, error: 'Lankningen har redan anvants' };
      }
        ...
        <Card>
          ...
          <CardContent>
            <p className="text-sm text-gray-600 mb-4 text-center">
              Ange din e-postadress sa skickar vi en lankning for att aterstalla ditt losenord.
            </p>
            <ForgotPasswordForm />
          </CardContent>
        </Card>
      ...
      return (
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <Button type="submit" className="w-full" disabled={isPending}>
            {isPending ? 'Skickar...' : 'Skicka atersta llningslankning'}
          </Button>
    ...
    export default async function ResetPasswordPage({
      ...
      if (!token) {
        return (
          <Card>
            <CardHeader>
              <CardTitle className="text-center">Ogiltig lankning</CardTitle>
            </CardHeader>
            <CardContent className="text-center">
              <p className="text-sm text-gray-600 mb-4">
                Lankningen for att aterstalla losenord saknas eller ar ogiltig.
              </p>
              <Link href="/forgot-password" className="text-blue-600 hover:underline">
                Begr en ny atersta llningslankning
              </Link>
            </CardContent>
          </Card>
        );
      ...
      if (!valid) {
        return (
          <Card>
            <CardHeader>
              <CardTitle className="text-center">Lankningen ar ogiltig</CardTitle>
            </CardHeader>
            <CardContent className="text-center">
    ...
    export function ResetPasswordForm({ token }: ResetPasswordFormProps) {
      ...
      if (success) {
        return (
          <div className="text-center space-y-4">
            <div className="p-3 bg-green-50 border border-green-200 rounded-md text-green-700 text-sm">
              Ditt losenord har atersta llts! Du dirigeras nu till inloggningen...
            </div>
            <Link href="/login" className="text-sm text-blue-600 hover:underline">
